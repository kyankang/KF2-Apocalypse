...
static function KF2GUIController GetGUIController( PlayerController PC )
{
    ...
    foreach PC.ChildActors(class'ApocGameMode.KF2GUIController',G)
    {
        if( !G.bIsInvalid )
        {
            break;
    ...
    if( G==None )
    {
        G = PC.Spawn(class'ApocGameMode.KF2GUIController',PC);
    }
...
simulated function HandleDrawMenu()
{
    if( HackConsole==None )
    {
        HackConsole = new(ClientViewport)class'ApocGameMode.KFGUIConsoleHack';
...
simulated final function SetMenuState( bool bActive )
{
    ...
    if( bActive )
    {
        if( CustomInput==None )
        {
            CustomInput = new (KFPlayerController(PlayerOwner)) class'ApocGameMode.KF2GUIInput';
